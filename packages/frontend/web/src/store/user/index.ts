...
import { UsersPermissions_User } from "@rrrcn/common-types/strapi/models/UsersPermissions_User";
import { setLocalStorageJWT } from "@/api/local-storage";
import { userRecentResultsSlice, userResultsSlice } from "@/store/results";
    ...
    logout: builder.mutation<null, any>({
      ...
      async onQueryStarted(args, { dispatch, queryFulfilled }) {
        try {
          ...
          dispatch(userRecentResultsSlice.actions.setUserResults([]));
          dispatch(userResultsSlice.actions.reset());
        } catch (e) {}
      },
    }),
