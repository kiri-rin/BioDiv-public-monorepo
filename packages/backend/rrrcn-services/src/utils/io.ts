...
export const writeScriptFeaturesArrayCsv = async (
  features: any[],
  fileName: string
) => {
  ...
  const res = await exportFeatureCollectionsToCsv(features);
  for (const chank of res.match(/(.|[\r\n]){1,100}/g) || []) {
    stream.write(chank);
  }
...
export const writeScriptFeaturesResult = async (
  ...
  return await writeScriptFeaturesArrayCsv(fc, fileName);
};
