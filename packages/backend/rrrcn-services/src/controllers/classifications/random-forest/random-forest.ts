...
export const randomForest = async (
...
): Promise<ClassificationApi.PostRandomForest.Response | undefined> => {
  if (!config.outputs) {
    return;
  }
  const {
  ...
  const raw_points = await getAllPoints(trainingPointsConfig);
  ...
  if (config.classificationSplits?.length && outputMode !== "CLASSIFICATION") {
    for (const split of config.classificationSplits) {
      ...
      await promise;
      for (const buffer of config.buffersPerAreaPoint || []) {
